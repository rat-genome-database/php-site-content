...
function makeReferenceURL($refRGDID) {
  return '/rgdweb/report/reference/main.html?id='.$refRGDID;
}
...
*/
   ...
   $jsonData = json_decode(file_get_contents('https://github.com/login/oauth/access_token?client_id=ee483d03b1806882b4b2&client_secret=cdb0f0faee2f2f9ea3e552e5997d3d5ea3ffbcb4&redirect_uri=https://dev.rgd.mcw.edu/rgdCuration/&code='.$_GET['code']));
...
/**
 * Create an HTML link to colored indicator image based on status of a term  1 or 0 ONT_TERMS.IS_OBSOLETE
 */
...
function getSpeciesNameAndAll( $speciesTypeKey,$includefor=false,$includespace=true ) {
   $toReturn='';
   $space='';
   if ($includespace)
   {
  ...
  switch ($speciesTypeKey) {
    case '1' :
      return $toReturn.'Human'.$space;
      break;
    case '3' :
      return $toReturn.'Rat'.$space;
      break;
    case '2' :
      return $toReturn.'Mouse'.$space;
      break;
    case '4' :
    ...
    case '1,2,3':
      ...
      break;
    default :
      return $toReturn."Undefined Species".$space;
  }
}
/**
...
/**
 * Returns one month ago plus one day
 ...
 */
 function getLastMonth()
 {
    ...
    {
        return date('m/d/Y',gettimeofday(true)-( ( 86400 * 30 ) +86400) );
    }
    else
    {
...
function getManualForTitle($pipe,$adverb=false)
{
    if ($pipe!='on'&&!$adverb)
    {
        return ' Manual';
...
function getEvidenceArrayForDropDown($not=true,$annot='GO')
{
    //eco($annot,2);
    if ($annot[0]=='G')
    {
    ...
    foreach ($re as $r)
    {
        ...
        if ($not)
        {
            $toReturn["and f.evidence!='$r'"]="non-$r";
        }
        else
        {
            $toReturn["'$r'"]="$r";
...
{
    $string=substr($string,0,18).substr($string,26,2);
    return strtotime($string);
}
function getLastRun($date,$dump=false)
{
...
function getSpellingSuggestions($term) {

  $suggestions = array();

  // certain numbers cause them to return invalid xml (ie, they're missing a closing xml tag)
  // so don't even try to send numbers
  // WR notified Tao Tao at NCBI about this on 11/9/07 (he said he'd notify the developers)
  if (is_numeric($term)) {
    return $suggestions;
  }
  ...
  if (isReallySet((string)$correction->CorrectedQuery)) {
    $suggestions[] = $correction->CorrectedQuery;
  }
 ...
 */
function fetchRecordsEx($sql, $params)
{
  $dbconn = getNamedConnection($dbname);
  $stmt = $dbconn->Prepare($sql);
  $rs = &$dbconn->Execute($stmt, $params);
  checkError($dbconn);
  $items = &$rs->GetRows();
  checkError($dbconn);
  return $items;
